...
func (s *Scheduler) fitInCertainDevice(node *NodeUsage, request util.ContainerDeviceRequest, annos map[string]string, pod *corev1.Pod, allocated *util.PodDevices) (bool, map[string]util.ContainerDevices, string) {
	k := request
	originReq := k.Nums
	prevnuma := -1
...
func (s *Scheduler) fitInDevices(node *NodeUsage, requests util.ContainerDeviceRequests, annos map[string]string, pod *corev1.Pod, devinput *util.PodDevices) (bool, string) {
	...
	for _, k := range requests {
		...
		fit, tmpDevs, reason := s.fitInCertainDevice(node, k, annos, pod, devinput)
		if fit {
	...
	for nodeID, node := range *nodes {
		...
		go func(nodeID string, node *NodeUsage) {
			...
			for ctrid, n := range nums {
				...
				fit, reason := s.fitInDevices(node, n, annos, task, &score.Devices)
				ctrfit = fit
				if !fit {
